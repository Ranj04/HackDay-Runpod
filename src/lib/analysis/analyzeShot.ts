// The full analysis entry point: capture -> metrics -> flaws -> score -> echo.
 ...
 * release-aligned reference echo projected onto the user for overlay.
 */
export const analyzeShot: AnalyzeShot = async (capture): Promise<AnalysisResult> => {
  ...
  const echoRef = alignToReference(capture, REFERENCE_CAPTURE, metrics.releaseFrameIndex ?? 0);
  return { capture, metrics, topFlaw, allFlaws, score, echoRef };
};
