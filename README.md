...
2. **Pose (Runpod Flash)** — the clip is sent to a GPU `@Endpoint` that runs pose estimation server-side and returns per-frame keypoints. Fanning these endpoints out is what lets Ghost scale past one device.
...
- **Next.js + Tailwind + shadcn/ui** — the app itself (capture UI, ghost-overlay canvas, results, history).
...
- ✅ **Analysis, scoring, ghost-overlay** — working, pure TypeScript.
